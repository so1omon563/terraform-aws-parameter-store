...
run "verify_parameter_outputs_plan" {
  command = plan
  assert {
    condition     = module.parameter.parameter.tags_all.example == "true" && module.parameter.parameter.tags_all.environment == "dev" && module.parameter.parameter.tags_all.terraform == "true"
